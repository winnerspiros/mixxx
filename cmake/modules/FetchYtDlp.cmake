...
set(YTDLP_VERSION "2026.03.17"
    CACHE STRING "Pinned yt-dlp release tag to bundle inside Mixxx")

if(WIN32)
  set(_ytdlp_asset "yt-dlp.exe")
  set(_ytdlp_sha256 "3db811b366b2da47337d2fcfdfe5bbd9a258dad3f350c54974f005df115a1545")
  set(_ytdlp_install_name "yt-dlp.exe")
elseif(APPLE)
  set(_ytdlp_asset "yt-dlp_macos")
  set(_ytdlp_sha256 "e80c47b3ce712acee51d5e3d4eace2d181b44d38f1942c3a32e3c7ff53cd9ed5")
  set(_ytdlp_install_name "yt-dlp")
elseif(UNIX AND NOT ANDROID)
  set(_ytdlp_asset "yt-dlp_linux")
  set(_ytdlp_sha256 "c2b0189f581fe4a2ddd41954f1bcb7d327db04b07ed0dea97e4f1b3e09b5dd8e")
  set(_ytdlp_install_name "yt-dlp")
else()
  message(STATUS "yt-dlp bundling skipped: unsupported platform")
  set(YTDLP_BINARY_PATH "" CACHE FILEPATH "yt-dlp binary to install with Mixxx" FORCE)
  return()
endif()

set(_ytdlp_cache_dir "${CMAKE_BINARY_DIR}/_deps/yt-dlp-${YTDLP_VERSION}")
set(_ytdlp_path "${_ytdlp_cache_dir}/${_ytdlp_install_name}")
set(_ytdlp_url "https://github.com/yt-dlp/yt-dlp/releases/download/${YTDLP_VERSION}/${_ytdlp_asset}")
...
if(EXISTS "${_ytdlp_path}")
  ...
  if(_have_sha STREQUAL _ytdlp_sha256)
    ...
    message(STATUS "Cached yt-dlp at ${_ytdlp_path} has wrong SHA-256, re-downloading")
...
if(_need_download)
  ...
  while(_attempt LESS_EQUAL _max_attempts AND NOT _ytdlp_ok)
    message(STATUS "Downloading yt-dlp ${YTDLP_VERSION} (${_ytdlp_asset}), attempt ${_attempt}/${_max_attempts}")
    file(DOWNLOAD
      "${_ytdlp_url}"
      "${_ytdlp_path}"
...
set(YTDLP_BINARY_PATH "${_ytdlp_path}"
    CACHE FILEPATH "yt-dlp binary to install with Mixxx" FORCE)
message(STATUS "Mixxx will bundle yt-dlp ${YTDLP_VERSION} from ${YTDLP_BINARY_PATH}")
